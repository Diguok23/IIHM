...
export default function IntasendPaymentVerificationPage() {
  ...
  useEffect(() => {
    ...
    const verifyPayment = async () => {
      try {
        const response = await fetch(`/api/intasend/verify?invoiceId=${invoiceId}`)
        ...
        if (isMounted) {
          if (data.status && data.data) {
            setPaymentStatus(data.data.state === "COMPLETED" ? "COMPLETED" : data.data.state === "FAILED" ? "FAILED" : "PENDING")
                  ...
                  {paymentDetails && (
                    <div className="bg-gray-50 p-4 rounded-lg border">
                      ...
                      <div className="space-y-1 text-sm">
                        <p>
                          <span className="font-medium">Amount:</span> {formatCurrency(paymentDetails.amount)}
                        </p>
                        <p>
                          <span className="font-medium">Reference:</span> {paymentDetails.id}
                        </p>
                        <p>
                        ...
                        <p>
                          ...
                          <span className={getStatusColor(paymentDetails.state)}>{paymentDetails.state}</span>
                        </p>
                      </div>
                    </div>
                  )}
